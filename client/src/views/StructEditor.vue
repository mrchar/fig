...
const { isFetching, data: completionResult, execute: executeCompletion } = api.completion.useCompleteStruct(query)
...
<template>
  <Form class="w-full h-full p-4 flex flex-col gap-2">
    <FormItem label="名称">
      <Input class="w-full" v-model="name" placeholder="请输入格式名称" />
    </FormItem>
    <FormItem>
      <template #label>
